...
def plot_lines_41d(latency_file: Path, cpu_file: Path, cores: int = 1, **kwargs):
    ...
    qps_max = 100 if cores == 1 else 200
    ...
    all_lines = [lines1[0], lines2[0], lines1[1]]  # Change order to show two lines first in legend, the SLO line
    all_labels = [labels1[0], labels2[0], labels1[1]]
    ax1.legend(all_lines, all_labels, loc="upper left", bbox_to_anchor=(0, 0.97))
...
def plot_A(plot_data: Part4PlotData, question: int, run: int):
    ...
    for tick in ax1_lat.get_yticklabels():
        tick.set_fontweight("bold")
    ax1_lat.yaxis.set_label_coords(0.115, 1.02)
    ...
    ax2.legend(loc="lower right", ncols=4)
...
def plot_scheduler(question: int, run: int, qps_interval: int=10):
    ...
    plot_data = prepare_part4_plot_data(
        parsed_logfile, memcached_df, qps_interval=qps_interval, question=3, run=run
    )
...
def main():
    ...
    # plot_lines_41d(Path("data/task4/Q1D/cpu1_memcached.csv"), Path("data/task4/Q1D/cpu1_stats.csv"), cores=1, title="QPS vs P95 Latency and CPU Utilization (C = 1, T = 2)")
    # plot_lines_41d(Path("data/task4/Q1D/cpu2_memcached.csv"), Path("data/task4/Q1D/cpu2_stats.csv"), cores=2, title="QPS vs P95 Latency and CPU Utilization (C = 2, T = 2)")

    # Scheduler plots
    plot_scheduler(question=3, run=1, qps_interval=10)
